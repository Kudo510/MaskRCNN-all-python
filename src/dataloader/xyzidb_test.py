...
class XYZIDBTest(torch.utils.data.Dataset):
    def __init__(self, root = "xyz_splits/val", transforms=True):
        ...
        for folder in folders:
            # photoneo_images = glob.glob(os.path.join(folder, "gray_photoneo/*.png"))
            if "val" in self.root:
                photoneo_images = glob.glob(os.path.join(folder, "gray_photoneo/*.png"))
            else:
                photoneo_images = glob.glob(os.path.join(folder, "rgb/*.png"))
            for img in photoneo_images:
    ...
    def __getitem__(self, idx):
        ...
        if "gray_photoneo" in img_path:
            mask_path = os.path.join(img_path.split("gray_photoneo")[0], "mask_visib_photoneo")
        else:
            mask_path = os.path.join(img_path.split("rgb")[0], "mask_visib")
